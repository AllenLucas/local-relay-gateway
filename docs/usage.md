...
这份文档面向日常维护本地网关的操作者，重点是 Windows 优先的启动、站点配置、模型映射和客户端接入流程。  
This guide is for day-to-day operators of the local gateway, with a Windows-first focus on startup, station configuration, model mappings, and client integration.
...
| --- | --- | --- |
| `LRG_LOCAL_API_KEY` | `change-me-local-key` | 本地网关认证密钥；OpenAI 本地入口读 `Authorization: Bearer`，Anthropic 本地入口读 `x-api-key` / Local gateway auth key; the OpenAI local entrypoint reads `Authorization: Bearer` and the Anthropic local entrypoint reads `x-api-key` |
...
如果要增加备用站点，把 `priority` 降低，例如 `50`，其余做同样配置即可。  
For a backup station, lower `priority` to something like `50` and fill the rest the same way.
...
3. 先为 `openai` 创建一个映射，再为 `anthropic` 创建一个映射。  
   Create one `openai` mapping first, then one `anthropic` mapping.
4. 保持 `Enabled` 勾选。  
   Keep `Enabled` checked.
...
- `Stations`：新增站点并查看当前已保存的基础配置。 / `Stations`: create stations and review saved base settings.
...
- 当前后台健康检查固定每 15 秒执行一次，对 `OpenAIBaseURL + /models` 发起 GET，请求超时固定为 5 秒。 / The current background health loop runs every 15 seconds, sending a GET to `OpenAIBaseURL + /models` with a fixed 5 second timeout.
...
先确认客户端环境变量中的密钥和 `LRG_LOCAL_API_KEY` 完全一致。OpenAI 本地入口读 `Authorization: Bearer`，Anthropic 本地入口读 `x-api-key`。  
First confirm that the client-side key exactly matches `LRG_LOCAL_API_KEY`. The OpenAI local route reads `Authorization: Bearer`, while the Anthropic local route reads `x-api-key`.
...
确认你不只是填了 OpenAI 的上游字段；同一条站点记录里还必须填 `anthropic_base_url` 和 `anthropic_api_key`，并另外创建 `anthropic` 协议映射。  
Make sure you did not only fill the OpenAI fields. The same station record must also include `anthropic_base_url` and `anthropic_api_key`, plus a separate `anthropic` mapping.

**状态页里长期显示 `cooldown`。**  
先检查上游是否持续返回 `429` 或 `5xx`，再检查 `openai_base_url` 是否能正确响应 `GET .../models`。  
First check whether the upstream keeps returning `429` or `5xx`, then verify that `openai_base_url` responds correctly to `GET .../models`.
